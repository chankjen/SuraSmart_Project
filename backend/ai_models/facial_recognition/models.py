...
class MissingPerson(models.Model):
    ...
    STATUS_CHOICES = (
        ('REPORTED', _('Reported')),
        ('RAISED', _('Raised to Police')),
        ('UNDER_INVESTIGATION', _('Under Investigation')),
        ('ANALYZED', _('Analyzed by Police')),
        ('ESCALATED', _('Escalated to Government')),
        ('GOVERNMENT_REVIEW', _('In Government Review')),
    ...
    case_assigned = models.BooleanField(default=False)
    ...
    # Report fields
    police_analysis_report = models.TextField(blank=True, null=True)
    government_analysis_report = models.TextField(blank=True, null=True)
    ...
    retention_policy_type = models.CharField(
        max_length=20,
        choices=(('standard', _('Standard (30 days)')), ('criminal', _('Criminal (5 years)'))),
        default='standard'
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.status == 'CLOSED' and not self.resolved_at:
            ...
            # Tiered retention policy based on type
            if self.retention_policy_type == 'standard':
                # Purge biometric data after 30 days
                self.retention_expiry_date = self.resolved_at + timedelta(days=30)
            else:
                # Criminal cases retain for 5 years
                self.retention_expiry_date = self.resolved_at + timedelta(days=5*365)
...
class JurisdictionConfig(models.Model):
    """Configuration for jurisdiction-specific rules, such as retention policies."""
    jurisdiction_code = models.CharField(max_length=10, unique=True, help_text=_('e.g., KE, EU, US'))
    ...
    class Meta:
        verbose_name = _('Jurisdiction Config')
        verbose_name_plural = _('Jurisdiction Configs')
        
    def __str__(self):
        return f'{self.jurisdiction_code} Configuration'


class OfflineSignatureQueue(models.Model):
    """Queue for storing case closure signatures collected offline."""
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    case = models.ForeignKey(MissingPerson, on_delete=models.CASCADE, related_name='offline_signatures')
    officer = models.ForeignKey(User, on_delete=models.CASCADE, related_name='offline_signatures')
    
    signature_payload = models.JSONField(help_text=_('Cryptographic signature data (WebAuthn/FIDO2)'))
    signed_at = models.DateTimeField(help_text=_('When the signature was created offline'))
    nonce = models.CharField(max_length=64, unique=True, help_text=_('Unique nonce to prevent replay attacks'))
    ...
    requires_manual_review = models.BooleanField(
        default=False, 
        help_text=_('Flagged true if case was modified online while offline')
    )
    ...
    class Meta:
        ordering = ['-created_at']
        verbose_name = _('Offline Signature')
        verbose_name_plural = _('Offline Signatures')
        
    def __str__(self):
        return f'Offline Signature for {self.case.full_name} by {self.officer.username}'
